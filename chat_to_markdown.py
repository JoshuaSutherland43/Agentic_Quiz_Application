    ...
    if isinstance(part, dict):
        ...
        if "kind" in part:
            ...
            if (
                ...
                and "value" in part["content"]
            ):
                return f"*{part['content']['value']}*"
            elif (
                "invocationMessage" in part
                and isinstance(part["invocationMessage"], dict)
                and "value" in part["invocationMessage"]
            ):
                return f"*{part['invocationMessage']['value']}*"
            elif (
                "pastTenseMessage" in part
                and isinstance(part["pastTenseMessage"], dict)
                and "value" in part["pastTenseMessage"]
            ):
                return f"*{part['pastTenseMessage']['value']}*"

        # Skip objects with internal IDs, metadata structure, or inline references
        if (
            ("id" in part and ("kind" in part or "$mid" in part))
            or "$mid" in part
            or "inlineReference" in part
        ):
            return ""
    ...
    for var in variables:
        ...
        if kind == "promptFile" and origin_label:
            ...
            if "github.copilot.chat." in origin_label:
                label_part = origin_label.split("github.copilot.chat.")[-1].split(" ")[
                    0
                ]
...
def format_tool_invocation_details(
    tool_data: Dict[str, Any],
    tool_call_results: Dict[str, Any] = None,
    tool_call_rounds: List[Dict[str, Any]] = None,
) -> str:
    ...
    if "[](file://" in invocation_msg:
        ...
        if file_match:
            ...
            remaining_text = invocation_msg[
                invocation_msg.find(file_match.group(0)) + len(file_match.group(0)) :
            ]
            ...
            if isinstance(round_data, dict) and "toolCalls" in round_data:
                for tool_call in round_data["toolCalls"]:
                    if isinstance(tool_call, dict):
                        ...
                        if (
                            tool_name == "read_file"
                            and "Read" in original_invocation_msg
                        ):
                            ...
                            if isinstance(arguments, str):
                                try:
                                    ...
                                    if (
                                        file_path
                                        and file_path in original_invocation_msg
                                    ):
    ...
    try:
        ...
        if output_data and isinstance(output_data, list) and output_data:
            output_value = (
                output_data[0].get("value", "")
                if isinstance(output_data[0], dict)
                else str(output_data[0])
            )
    ...
    try:
        ...
        lang = (
            ...
            if file_ext == ".md"
                ...
                "python"
                if file_ext == ".py"
                else ("json" if file_ext == ".json" else "")
            )
        )
            ...
            if edit_range:
                ...
                if start_line and end_line:
                    if start_line == end_line:
                        lines.append(
                            f"  <p><strong>Modified line {start_line}:</strong></p>"
                        )
                    else:
                        lines.append(
                            f"  <p><strong>Modified lines {start_line}-{end_line}:</strong></p>"
                        )
                    lines.append(f"")
            ...
            for i, edit in enumerate(all_edits):
                ...
                if edit_range:
                    ...
                    if start_line and end_line:
                        if start_line == end_line:
                            lines.append(
                                f"  <p><strong>Line {start_line}:</strong></p>"
                            )
                        else:
                            lines.append(
                                f"  <p><strong>Lines {start_line}-{end_line}:</strong></p>"
                            )
                        lines.append(f"")
        ...
        # If there are many edits, group them into a single consolidated block
            ...
            lines.append(
                f"  <p><strong>Multiple file changes ({len(all_edits)} edits)</strong></p>"
            )
            lines.append(f"")
            ...
            while i < len(sorted_edits):
                ...
                while j < len(sorted_edits):
                    ...
                    prev_end_line = (
                        prev_edit_range.get("endLineNumber", prev_line_num)
                        if prev_edit_range
                        else prev_line_num
                    )
                    ...
                    if (
                        next_line_num != 0
                        and prev_end_line != 0
                        and next_line_num <= prev_end_line + 2
                    ):
                ...
                if len(consecutive_edits) > 1:
                    ...
                    last_line = (
                        last_edit_range.get("endLineNumber", consecutive_edits[-1][0])
                        if last_edit_range
                        else consecutive_edits[-1][0]
                    )
                    ...
                    for _, cons_edit in consecutive_edits:
                        ...
                        if cons_text is not None:
                            ...
                            consecutive_text_parts.append(
                                cons_text
                            )  # Include even empty strings to preserve spacing
...
def process_special_markers(
    ...
    text = re.sub(
        r"__TEXT_EDIT_GROUP__(.*?)__TEXT_EDIT_GROUP__",
        replace_text_edit_group,
        text,
        flags=re.DOTALL,
    )
    ...
    def replace_tool_invocation(match):
        try:
            ...
            return format_tool_invocation_details(
                tool_data, tool_call_results, tool_call_rounds
            )
        except:
            return ""

    text = re.sub(
        r"__TOOL_INVOCATION__(.*?)__TOOL_INVOCATION__",
        replace_tool_invocation,
        text,
        flags=re.DOTALL,
    )
    ...
    text = re.sub(
        r"__PROGRESS_TASK__(.*?)__PROGRESS_TASK__",
        replace_progress_task,
        text,
        flags=re.DOTALL,
    )
...
def parse_chat_log(chat_data: Dict[str, Any]) -> str:
    ...
    md_lines.append(
        f"<br>**Assistant:** {chat_data.get('responderUsername', 'GitHub Copilot')}"
    )
    md_lines.append("")
        ...
        if response or (
            ...
            if response:
                            ...
                            for round_data in tool_call_rounds:
                                if isinstance(round_data, dict):
                                    ...
                                    if "response" in round_data:
                                        ...
                                        if (
                                            isinstance(round_response, str)
                                            and round_response.strip()
                                        ):
                                            tool_responses.append(
                                                round_response.strip()
                                            )
                ...
                if response_parts:
                    ...
                    incremental_response = process_special_markers(
                        incremental_response, tool_call_results, tool_call_rounds
                    )

                    # Use the incremental response if it has more detail, otherwise use consolidated
                    if (
                        "__TOOL_INVOCATION__" in "\n".join(response_parts)
                        or "__TEXT_EDIT_GROUP__" in "\n".join(response_parts)
                        or not consolidated_response.strip()
                    ):
